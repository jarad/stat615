...
plot(dropFirst(gdpSmooth$s[, c(1, 3)]), oma.multi = c(2,0,1,0),
     mar.multi = c(0, 2.1, 0, 2.1), cex.axis = 1,
...
capm <- read.table("P.dat", header = TRUE)
capm.ts <- ts(capm, start = c(1978, 1), frequency = 12)
colnames(capm)
par(cex = 0.5)
require(zoo)
plot(as.zoo(capm.ts), main = "", xlab = "", cex.lab = 1,
